...
class ErrorBoundary extends React.Component<{children: React.ReactNode}, {hasError: boolean, error: any}> {
  ...
  render() {
    if (this.state.hasError) {
      return (
        <div style={{ padding: '20px', color: 'red', backgroundColor: '#fff', height: '100vh', fontFamily: 'monospace' }}>
          ...
          <p>Por favor toma una captura de pantalla de este error y envíamelo.</p>
        </div>
      );
    }
...
function AppContent() {
  ...
  const getFinalEq = (traj: any) => {
     ...
     if(!last || !last.position || !last.velocityVector) return '';
  ...
  const safeTMax = isNaN(tMax) || tMax === 0 ? 1 : tMax;
  const currentM1Pos = m1Traj[Math.min(Math.floor((safeTime/safeTMax)*(m1Traj.length-1)), m1Traj.length-1)]?.position || [0,0,0];
  const currentM2Pos = m2Traj[Math.min(Math.floor((safeTime/safeTMax)*(m2Traj.length-1)), m2Traj.length-1)]?.position || [0,0,0];
    ...
    <div className="app-grid bg-[#020610] text-[#e2e8f0] font-sans selection:bg-[#00f2ff]/30">
      ...
      <header className="expert-header flex items-center justify-between px-6">
        ...
        <div className="flex flex-col items-center justify-center border border-emerald-500/30 rounded px-8 py-1 bg-emerald-500/10">
          <span className="text-[10px] text-emerald-400 font-bold tracking-widest">{isRunning ? "SIMULACIÓN EN CURSO" : safeTime >= safeTMax ? "INTERCEPCIÓN COMPLETADA" : "SISTEMA EN ESPERA"}</span>
          <span className="text-white font-mono text-sm">TIEMPO: 00:00:{safeTime.toFixed(3).padStart(6, '0')}</span>
        </div>
      ...
      <main className="expert-viewport bg-sky-900/20">
        <div className="absolute top-4 left-4 z-10 space-y-4 pointer-events-none">
          <div className="bg-[#020610]/80 border border-[#1a3a4a] p-4 rounded text-xs w-64 shadow-xl backdrop-blur-md pointer-events-auto">
          ...
          {/* MATH VECTOR PANEL */}
          <div className="bg-[#020610]/80 border border-[#1a3a4a] p-4 rounded text-xs w-[340px] shadow-xl backdrop-blur-md pointer-events-auto">
             ...
             <div className="space-y-4">
                <div>
                   ...
                   <div className="font-mono text-[9px] text-white mt-1 border-t border-white/10 pt-1">
                      P(t={safeTime.toFixed(1)}): ⟨{currentM1Pos.map(n=>n.toFixed(1)).join(', ')}⟩
                   </div>
                </div>
                <div>
                   ...
                   <div className="font-mono text-[9px] text-white mt-1 border-t border-white/10 pt-1">
                      P(t={safeTime.toFixed(1)}): ⟨{currentM2Pos.map(n=>n.toFixed(1)).join(', ')}⟩
                   </div>
                </div>
             </div>
          </div>
        ...
        <Canvas camera={{ position: [0, 40, 180], fov: 45 }} style={{ cursor: mode !== 'NONE' ? 'crosshair' : 'default' }}>
          ...
          <HtmlLabels p1={parseVec(m1P)} p2={parseVec(m2P)} target={parseVec(targetP)} time={safeTime} tMax={safeTMax} />

          <Missile points={m1Traj} currentTime={safeTime} tMax={safeTMax} color="#ff2200" isDashed />
          <Missile points={m2Traj} currentTime={safeTime} tMax={safeTMax} color="#0077ff" isDashed />
      ...
      <aside className="expert-sidebar p-5 space-y-8 overflow-y-auto custom-scrollbar text-xs">
         <div>
            ...
            <div className="space-y-3 font-mono">
               ...
               <div className="flex items-center text-emerald-400"><div className={`w-2 h-2 rounded-full mr-3 ${safeTime >= safeTMax ? 'bg-emerald-400' : 'bg-slate-600'}`}></div> {safeTime >= safeTMax ? 'INTERCEPCIÓN: EXITOSA' : 'INTERCEPCIÓN: PENDIENTE'}</div>
            </div>
         </div>
        ...
        <table className="data-table">
          ...
          <tbody>
            ...
            <tr>
              ...
              <td>({formatVector(m1Traj[m1Traj.length-1]?.velocityVector || [0,0,0])})</td>
              <td>v⃗(t) = v⃗₀ + a⃗t</td>
              <td className="text-slate-400">({m2V})</td>
              <td>({formatVector(m2Traj[m2Traj.length-1]?.velocityVector || [0,0,0])})</td>
            </tr>
            <tr>
