...
export const Intruder: React.FC<{ points: TrajectoryPoint[], isActive?: boolean }> = ({ points, isActive }) => {
  ...
  useFrame((state) => {
    if (meshRef.current && isActive) {
      // In sync with interceptor (alpha 0..1 over 10s roughly, or matching the 5s/8s cycle)
      // For simplicity, we use the same clock logic
      const t = (state.clock.elapsedTime % 10) / 10;
      const idx = Math.floor(t * (points.length - 1));
      const p = points[idx]?.position;
      if (idx < points.length && p) {
        meshRef.current.position.set(p[0], p[1], p[2]);
        // Simple forward orientation logic
        if (idx < points.length - 1) {
          const nextP = points[idx+1].position;
          const dir = new THREE.Vector3(nextP[0]-p[0], nextP[1]-p[1], nextP[2]-p[2]).normalize();
