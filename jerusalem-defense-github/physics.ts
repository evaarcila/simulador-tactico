    ...
    <group>
      ...
      {hasImpacted && (
        <group position={targetPos}>
          ...
          <Sphere args={[4.5, 16, 16]}>
            <meshBasicMaterial color="#ff4400" transparent opacity={0.6} wireframe />
          </Sphere>
          <pointLight intensity={200} color="#ff8800" distance={100} />
          <pointLight intensity={100} color="#0077ff" distance={80} />
          
          {/* Debris Particles */}
          <Sparkles count={800} scale={30} size={5} speed={2} opacity={0.8} color="#ffffff" />
          <Sparkles count={400} scale={20} size={15} speed={1} opacity={1} color="#ff2200" />
          <Sparkles count={400} scale={20} size={15} speed={1} opacity={1} color="#0077ff" />
...
export const Globe: React.FC<{ 
  ...
  basePos2?: [number, number, number],
  onGroundClick?: (pos: [number, number, number]) => void
}> = ({ basePos = [0, -5, 0], basePos2, onGroundClick }) => {
  ...
  return (
    <group>
      ...
      {/* Jerusalem Tactical Base 2 */}
      {basePos2 && (
        <group position={basePos2}>
          <mesh position={[0, -4.5, 0]}>
            <cylinderGeometry args={[8, 10, 3, 6]} />
            <meshStandardMaterial color="#000" emissive="#0077ff" emissiveIntensity={5} wireframe />
          </mesh>
          <pointLight position={[0, 12, 0]} intensity={45} color="#0077ff" distance={300} />
          
          {[1, 2.5, 4.5].map(i => (
             <Torus key={i} args={[12 * i, 0.15, 16, 120]} rotation={[Math.PI/2, 0, 0]}>
                <meshBasicMaterial color="#0077ff" transparent opacity={0.5 / i} />
             </Torus>
          ))}
        </group>
      )}
